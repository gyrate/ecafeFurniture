...
Sui.data.TreeStore = Sui.extend(Sui.data.Store, {
    ...
    insertRecordNotSortData : function(datas, sortFunction) {
        ...
        for (var i = 0; i < rootNodes.length; i++) {
            rootNodes[i].preorderTraversal(function(node) {
                ...
                if (!Sui.isDefinedAndNotNull(taskDTO[thisStore.expandedName])) {

                    var parentId = taskDTO[thisStore.parentIdName];
                    if (parentId) {
                        taskDTO[thisStore.expandedName] = idNodeMap[parentId].getData()[thisStore.expandedName];
                    } else {
                        taskDTO[thisStore.expandedName] = 'true';
                    }
                }
    ...
    /**
     *  在某个索引前插入{Sui.data.TreeRecord}类型的数据
 ...
 * </code></pre>
 */
Sui.table.TreeTable = Sui.extend(Sui.table.Table, {
    ...
    initConfig : function(config) {
        ...
        Sui.table.TreeTable.superclass.initConfig.apply(this, arguments);
    ...
    renderBody: function () {
        ...
        for (var i = j = 0; i < rowCount;i++) {
            //判断每一行是否需要渲染
            var record = this.store.getRecord(i);
            var parentRecord = record.getParentRecord();
            if ( !parentRecord || parentRecord.getFieldValue(this.store.expandedName) === 'true') {
                 this.renderRow(record, j++, rowCount);
                //record.setFieldValue(this.store.expandedName,'true');
            }else{
                record.setFieldValue(this.store.expandedName,'false');
            }
    ...
    renderBodyCell : function(cellValue, columnModel, tdJQ, record) {
        ...
        Sui.table.TreeTable.superclass.renderBodyCell.apply(this, arguments);
    ...
    updateFoldIconStyle : function(record, foldIcon) {
        if (record.isLeaf()) {
            ...
            if (! this.hasFoldClass(foldIcon)) {
                var expandedName = record.getFieldValue(this.store.expandedName);
                if(Sui.isDefinedAndNotNull(expandedName) ){
                    foldIcon.addClass( expandedName === 'true'? "sui_treetable_minus" : "sui_treetable_plus")
                }else{
                    foldIcon.addClass("sui_treetable_plus");
                }
    ...
    onFoldClick : function(e) {
        ...
        //如果该节点有子节点且没渲染,则创建它
        this.createRowsIfNotRender(rowIndex);
     ...
     * @param rowIndex
     */
    createRowsIfNotRender:function(rowIndex) {

        var nodeId = this.getRowElement(rowIndex).attr('nodeId');
        var record = this.store.findRecordByNameValue(this.store.idName, nodeId);
        var insertIndex =  rowIndex;
        //迭代
        var subRecordIndexs =  this.store.getChildRecordIndexs(record, true);

        for (var i = 0,len = subRecordIndexs.length; i < len; i++) {

            var srecord = this.store.getRecord(subRecordIndexs[i]);
            var parentRecord = srecord.getParentRecord();

            if( parentRecord == record || parentRecord.getFieldValue(this.store.expandedName) === 'true'){
                var nodeId = srecord.getFieldValue(this.store.idName);
                var trJQ = this.bodyElement.find('tr[nodeId=' + nodeId + ']');
                if (!trJQ.length) {
                    this.addRowDynamic({
                        record : srecord,
                        index: ++insertIndex
    ...
    onAddRow : function(event) {
        ...
        if ( !parentRecord || parentRecord.getFieldValue(this.store.expandedName) === 'true') {
            Sui.table.TreeTable.superclass.onAddRow.apply(this, arguments);
            ...
            var visible = false;
            var parentRowIndex = this.getTrDomIndex( record.getParentIndex() ) ;
            if (parentRowIndex == -1) {
                visible = true;
            } else {
                if (this.isRowVisible(parentRowIndex)) {
                    if (this.isRowFold(parentRowIndex)) {
                        visible = false;
                    } else {
                        visible = true;
                    }
    ...
    onRemoveRow : function(event) {
        Sui.table.TreeTable.superclass.onRemoveRow.apply(this, arguments);
    ...
    updateParentRecordFoldIconStyle:function(record) {
        ...
        if (parentRecord) {
            this.updateFoldIconStyle(parentRecord, this.getFoldElementByIdName(parentRecord.getId()));
        }
    },
    /**
    ...
    updateRowFoldState : function(rowIndex, isFold) {
        ...
        if (isFold) {
            //折叠
            var subRecordIndexs = this.store.getChildRecordIndexs(record, true);
            this.setRowsVisibleByRecordIndex(subRecordIndexs, !isFold);
        } else {
            //展开
            var subRecordIndexs = this.store.getChildRecordIndexs(record, false);
            if (subRecordIndexs) {
                this.setRowsVisibleByRecordIndex(subRecordIndexs, !isFold,rowIndex);
                //更新子节点的状态
                for (var i = 0; i < subRecordIndexs.length; i++) {
                    var childDomIndex = this.getTrDomIndex(subRecordIndexs[i]);
                    var isChildFold = this.isRowFold(childDomIndex);
                    if (! isChildFold) {
                        this.updateRowFoldState(childDomIndex, isChildFold);
                    }
                }
            }
        }
        record.setFieldValue(this.store.expandedName,!isFold.toString());
    },

    getTrDomIndex:function(recordIndex) {

        if (recordIndex > 0 && recordIndex < this.store.getCount()) {
            var nodeId = this.store.getRecord(recordIndex).getFieldValue(this.store.idName);
            return this.bodyElement.find('tr[nodeId=' + nodeId + ']').index();
        }
        return -1;
    },
    ...
    getFoldElementByIdName:function(nodeId){
        var tdJQ = this.bodyElement.find('tr[nodeId=' + nodeId + ']');
        return tdJQ.find(".sui_treetable_foldicon");
    },
    /**
     * 设置多行的可见度
    ...
    setRowsVisibleByRecordIndex:function(recordIndexs, visible){

        for (var i = 0,len = recordIndexs.length; i < len; i++) {
            var record = this.store.getRecord(recordIndexs[i]);
            var nodeId = record.getFieldValue(this.store.idName);
            this.bodyElement.find('tr[nodeId=' + nodeId + ']').toggle(visible);
        }
    },
    /**
     * 判断某行是否可见
    ...
    renderRow: function (record, index, rowCount) {
        ...
        if (index == 0) {
            ...
            var trJQ = $("<tr></tr>").insertAfter(this.getRowElement(Math.min(index - 1,this.bodyElement.children().length-1))).addClass(className);
        }
        trJQ.attr('nodeId',record.getFieldValue(this.store.idName));
